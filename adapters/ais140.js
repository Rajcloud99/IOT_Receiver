...
class adapter{
    ...
    parse_data(data) {
        data = data.toString();
       // console.log('ais-140',data);
       
        const parts = {};
        parts.org = data;
        data = data.split(',');
        //$LGN,,869867036977819,2.0ATRFID,0.00000000,N,0.00000000,E*
       ...
       // parts.time = data.substr(1, 12);
        ...
        switch (parts.cmd) {
            ...
            case '$HLM':
                ...
                parts.fVer = data[2];
                parts.device_id = parseInt(data[3]);
                parts.battery = parts.data[4];
                //Indicates flash memory % used
                parts.memory = parts.data[5];
                //Indicates packet frequency when ignition is on in seconds
                parts.packat_ignition_on = parts.data[6];
                //Indicates packet frequency when ignition is off in seconds
                parts.packat_ignition_off = parts.data[7];
                //Digital Input status order:[DIN3, DIN2, DIN1, DIN0] -> 0000
                parts.dig_input = parts.data[8];
                //Analog Input status -> 00
                parts.analog_input = parts.data[9];
                break;
        ...
        if(this.device.getUID() == this.device.logOne){
            console.log(this.device.logOne,parts.org);
            fs.appendFile('ais140'+this.device.logOne+'.txt', new Date() + ' : '+ parts.org+'\n', function (err) {});
        }
        return parts;
    }
    ...
    get_ping_data(msg_parts) {
        ...
        const data =  {
            real_time : str[5]=='L',
            device_id: this.device.getUID() || str[6],
            vehicle_no:str[7],
            gps_tracking:str[8] == 1,
            lat: parseFloat(str[11]),
            ns: str[12],
            lng: parseFloat(str[13]),
            ew: str[14],
            speed: parseInt(str[15]),
            course: parseInt(str[16]),
            satellites:parseInt(str[17]),
            altitude:str[18],
            pdop:str[19],
            hdopL:str[20],
            operator:str[21],
            ignition:parseInt(str[22]),
            power_off:str[23],
            input_voltage:str[24],
            battery_voltage:str[25],
            emergency:str[26],
            temper:str[27],
            gsm:str[28],
            mcc:parseInt(str[29]),
            ...
            input_state: str[34],
            out_state: str[35],
            analog_input1:str[36],
            analog_input2:str[37],
            frame_no:str[38],
            odometer: parseInt(str[39]),
            msc1:str[40],
            msc2:str[42],
            msc3:str[42],
            msc4:str[43],
