...
class adapter{
    ...
    parse_data(data) {
       ...
       // fs.appendFile('crxv5.txt',  new Date() + ' dataHex : '+ dataHex+'\n', function (err) {});
       // console.log(dataHex);
        const parts = {};
        ...
        switch (parts.cmd) {
            case '01':
                ...
                let imei = parts.data.substr(0,16);
                let dModel = parts.data.substr(16,4);
                let timezone_lng = parts.data.substr(20,4);
                parts.device_id = parseInt(imei);
                break;
    ...
    receive_alarm(msg_parts) {
       ...
       // console.log('V5 alarm ',this.device.getUID() , data.alarm_terminal,data.alarm_phone);
    ...
    getCommandCustom(flag, command) {
        ...
        const end = '0D0A';
        let fCmd = start + length + cmd + content + serial + error + end;
        console.log('crxv5 fCmd',fCmd);
        return fCmd;
    }
    ...
    sendCommand(type, param) {
        let command;
        let num;
        console.log('command crxv5 1',type,param);
        for (let i = 0; i < devices.length; i++) {
            if (devices[i].key === this.device.model_name) {
        ...
        console.log('command crxv5',command);
