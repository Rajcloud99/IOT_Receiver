...
class adapter{
	...
	parse_data(data) {
		...
		// fs.appendFile('tr06n.txt', dataHex+'\n', function (err) {});
	...
	receive_alarm(msg_parts) {
		...
		const data = {
			...
			gsm_signal_str: parseInt(str.substr(58, 2), 16) * 25,
		...
		if(cHr == ISTHr){
			//console.log('gt06n IST',data.device_id,datetime,new Date(datetime));
		}else if(cHr == UTCHr){
			datetime += ' +0000';//800 to 000
			//console.log('gt06n UTC',data.device_id,datetime,new Date(datetime));
		}else{//china time
			datetime += ' +0800';
		}	
	...
	get_ping_data(msg_parts) {
		...
		data.datetime = new Date(datetime).getTime();
		/*
		let diffMin = (new Date().getTime() - data.datetime)/60000;
		if(diffMin < 10){
